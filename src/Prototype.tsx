...
type Tile = { id: number; fruit: Fruit; special?: Special; foamLayers?: number };
type Pos = { row: number; col: number };
type SavedGame = { unlocked: number; boosters: { hammer: number; flower: number; swap: number }; best: Record<number, number> };

const FRUITS: Fruit[] = ["blueberry", "strawberry", "peach", "grape", "lemon", "apple"];
...
function loadSave(): SavedGame {
  try {
    ...
    return { unlocked: Math.max(1, Math.min(20, Number(saved.unlocked) || 1)), boosters: { hammer: Number(saved.boosters?.hammer) || 3, flower: Number(saved.boosters?.flower) || 3, swap: Number(saved.boosters?.swap) || 3 }, best: saved.best || {} };
  } catch { return { unlocked: 1, boosters: { hammer: 3, flower: 3, swap: 3 }, best: {} }; }
}

function makeTile(fruit?: Fruit): Tile {
  return { id: ++tileId, fruit: fruit ?? FRUITS[Math.floor(Math.random() * FRUITS.length)] };
}

function emptyBoard(foamTarget = 5): Tile[][] {
  let board: Tile[][] = [];
  do {
    board = Array.from({ length: BOARD_SIZE }, () => Array.from({ length: BOARD_SIZE }, () => makeTile()));
  } while (findMatches(board).length > 0);
  const cloudSlots: Pos[] = [
  ...
  cloudSlots.forEach((pos, index) => {
    if (remaining <= 0) return;
    const layers = Math.min(2, remaining - Math.max(0, cloudSlots.length - index - 1));
    board[pos.row][pos.col].foamLayers = Math.max(1, layers);
    remaining -= Math.max(1, layers);
  });
  board[4][4].special = "flower";
  return board;
}

function cloneBoard(board: Tile[][]) { return board.map((row) => row.map((tile) => ({ ...tile }))); }
function inside({ row, col }: Pos) { return row >= 0 && row < BOARD_SIZE && col >= 0 && col < BOARD_SIZE; }
function isAdjacent(a: Pos, b: Pos) { return Math.abs(a.row - b.row) + Math.abs(a.col - b.col) === 1; }
function key(pos: Pos) { return `${pos.row}:${pos.col}`; }
function samePos(a: Pos | null, b: Pos) { return Boolean(a && a.row === b.row && a.col === b.col); }
...
function findMatches(board: Tile[][]): Pos[][] {
  const groups: Pos[][] = [];
  for (let row = 0; row < BOARD_SIZE; row += 1) {
    let start = 0;
    for (let col = 1; col <= BOARD_SIZE; col += 1) {
      if (col < BOARD_SIZE && board[row][col].fruit === board[row][start].fruit) continue;
      if (col - start >= 3) groups.push(Array.from({ length: col - start }, (_, i) => ({ row, col: start + i })));
      start = col;
    }
  }
  for (let col = 0; col < BOARD_SIZE; col += 1) {
    ...
    for (let row = 1; row <= BOARD_SIZE; row += 1) {
      ...
      if (row - start >= 3) groups.push(Array.from({ length: row - start }, (_, i) => ({ row: start + i, col })));
      start = row;
    }
  }
  return groups;
...
export default function Prototype() {
  ...
  const [board, setBoard] = useState<Tile[][]>(() => emptyBoard(LEVELS[0].targets[1].count));
  ...
  const resetLevel = useCallback((nextIndex = levelIndex) => {
    ...
    setBoard(emptyBoard(next.targets[1].count)); setMoves(next.moves); setScore(0); setSelected(null); setBusy(false); setBooster(null);
    setProgress({ strawberry: 0, foam: 0 }); setMessage("准备好了吗？交换相邻水果开始消除！"); setShowResult(null);
  }, [levelIndex]);
  ...
  const goalsComplete = (nextProgress: { strawberry: number; foam: number }) =>
  ...
  const resolveBoard = useCallback(async (initial: Tile[][], preferred?: Pos) => {
    ...
    while (true) {
      ...
      toClear.forEach((value) => {
        ...
        if (current.foamLayers) { gainedFoam += 1; if (current.foamLayers > 1) { next[row][col] = { ...current, foamLayers: current.foamLayers - 1 }; cloudDamaged.add(value); } return; }
      ...
      for (let col = 0; col < BOARD_SIZE; col += 1) {
        const fallen = next.map((row, rowIndex) => ({ tile: row[col], rowIndex })).filter(({ rowIndex }) => !cleared.has(key({ row: rowIndex, col }))).map(({ tile }) => tile);
        const newColumn = Array.from({ length: BOARD_SIZE - fallen.length }, () => makeTile()).concat(fallen);
        newColumn.forEach((tile, row) => { next[row][col] = tile; });
      }
    ...
    if (totalScore) {
      ...
      setProgress((value) => {
        const nextProgress = { strawberry: value.strawberry + gainedStrawberry, foam: value.foam + gainedFoam };
        if (goalsComplete(nextProgress)) { window.setTimeout(() => { setShowResult("win"); setUnlocked((value) => Math.max(value, Math.min(20, levelIndex + 2))); if (audioEnabled.current) playTone("win"); }, 240); }
        return nextProgress;
      });
  ...
  const swapAndResolve = useCallback(async (a: Pos, b: Pos) => {
    ...
    const next = cloneBoard(board); [next[a.row][a.col], next[b.row][b.col]] = [next[b.row][b.col], next[a.row][a.col]];
    const directSpecial = next[a.row][a.col].special || next[b.row][b.col].special;
    if (!directSpecial && !findMatches(next).length) {
      setBoard(next); setMessage("这里没有可消除的组合，再试试！"); if (audioEnabled.current) playTone("lose");
      await new Promise((resolve) => window.setTimeout(resolve, 190)); setBoard(board); setBusy(false); return;
    }
    ...
    if (directSpecial && !findMatches(next).length) {
      ...
      target.forEach((value) => {
        ...
        if (tile.foamLayers) { foamHits += 1; if (tile.foamLayers > 1) { next[row][col] = { ...tile, foamLayers: tile.foamLayers - 1 }; cloudDamaged.add(value); } }
        else if (tile.fruit === "strawberry") strawberryHits += 1;
      });
      setClearing(target);
      await new Promise((resolve) => window.setTimeout(resolve, 210));
      for (let col = 0; col < BOARD_SIZE; col += 1) {
        const fallen = next.map((row, rowIndex) => ({ tile: row[col], rowIndex })).filter(({ rowIndex }) => !target.has(key({ row: rowIndex, col })) || cloudDamaged.has(key({ row: rowIndex, col }))).map(({ tile }) => tile);
        Array.from({ length: BOARD_SIZE - fallen.length }, () => makeTile()).concat(fallen).forEach((tile, row) => { next[row][col] = tile; });
      }
      setScore((value) => value + target.size * 70); setBoard(next); setClearing(new Set());
      setProgress((value) => ({ ...value, strawberry: Math.min(level.targets[0].count, value.strawberry + strawberryHits), foam: Math.min(level.targets[1].count, value.foam + foamHits) }));
  ...
  const useBooster = useCallback(async (pos: Pos) => {
    ...
    if (activeBooster === "swap") { setSelected(pos); setMessage("已选中第一个水果，再滑向相邻水果完成交换。"); setBooster(null); setBusy(false); return true; }
    if (activeBooster === "hammer") {
      ...
      await new Promise((resolve) => window.setTimeout(resolve, 230)); const tile = next[pos.row][pos.col]; next[pos.row][pos.col] = tile.foamLayers && tile.foamLayers > 1 ? { ...tile, foamLayers: tile.foamLayers - 1 } : makeTile(); setScore((value) => value + 90);
      if (hitFoam) setProgress((value) => ({ ...value, foam: Math.min(level.targets[1].count, value.foam + 1) }));
    }
    if (activeBooster === "flower") {
      ...
      let foamHits = 0;
      for (let row = 0; row < BOARD_SIZE; row += 1) for (let col = 0; col < BOARD_SIZE; col += 1) if (target.has(key({ row, col }))) { const tile = next[row][col]; if (tile.foamLayers) { foamHits += 1; next[row][col] = tile.foamLayers > 1 ? { ...tile, foamLayers: tile.foamLayers - 1 } : makeTile(); } else next[row][col] = makeTile(); }
      setScore((value) => value + target.size * 75);
      if (foamHits) setProgress((value) => ({ ...value, foam: Math.min(level.targets[1].count, value.foam + foamHits) }));
    }
    setBoosters((value) => ({ ...value, [activeBooster]: Math.max(0, value[activeBooster] - 1) }));
    setBoard(next); setClearing(new Set()); setBooster(null); setMessage(activeBooster === "hammer" ? "小锤子生效！" : "彩虹花魔法完成！"); await new Promise((resolve) => window.setTimeout(resolve, 170)); setBusy(false); return true;
  }, [board, booster, boosters, busy]);
  ...
  const onBoardPointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    dragStart.current = pos; setDragging(pos); event.currentTarget.setPointerCapture(event.pointerId);
  };
  const onBoardPointerUp = (event: PointerEvent<HTMLDivElement>) => {
    const from = dragStart.current; const to = tileFromPointer(event); dragStart.current = null; setDragging(null);
    if (!from || !to || busy) return;
    const dx = to.col - from.col; const dy = to.row - from.row;
    if (Math.abs(dx) + Math.abs(dy) === 1) { void swapAndResolve(from, to); return; }
    void clickTile(from);
  };
  const onBoardPointerCancel = () => { dragStart.current = null; setDragging(null); };

  useEffect(() => { if (moves === 0 && !showResult && !goalsComplete(progress)) window.setTimeout(() => { setShowResult("lose"); if (audioEnabled.current) playTone("lose"); }, 260); }, [moves, progress, showResult]);
